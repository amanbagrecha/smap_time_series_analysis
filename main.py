...
def plot_time_series(dataframe):
  ...
  fig.add_trace(go.Scatter(x=dataframe.iloc[:,0], y=dataframe.iloc[:,1],
            mode='markers+lines',connectgaps=True,  
            name='DESC Pass'))



  fig.update_layout(
      title=dict(xref='paper', yref='paper',
                              xanchor='left', yanchor='bottom',
                              text=f"Soil Moisture: {dataframe.time.min().strftime('%d %b')} to {dataframe.time.max().strftime('%d %b, %Y')}",
                              font=dict(family='Arial',
                                        size=30,
                                        color='rgb(37,37,37)')),
      yaxis_title = dict(text="'$cm^3 cm^{-3}$'",
                              font=dict(family='Arial',
                                        size=12,
                                        color='rgb(150,150,150)')),
      xaxis_title = dict(text="Source: O'Neill, et al. NASA NSIDC DAAC ",
                              font=dict(family='Arial',
                                        size=12,
                                        color='rgb(150,150,150)')),
      xaxis=dict(
...
if __name__ == '__main__':
  ...
  df_merged = df_merged.sort_values(by='time').set_index('time')
  df_merged = df_merged.apply(np.nanmean, axis=1).reset_index()
